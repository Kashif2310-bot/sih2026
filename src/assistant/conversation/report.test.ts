...
describe('buildPersonalizedReport', () => {
  it('assembles every section from existing deterministic data, with no LLM call and no invented facts', () => {
    ...
    expect(report.citizenSnapshot.businessSector).toEqual({ value: 'dairy', source: 'user_provided', confidence: undefined, capturedAt: '2026-01-01T00:00:00.000Z' })
    ...
    // The AI-narrative seam exists but is never filled in by the deterministic builder.
  ...
  it('correctly separates verified-local from live-official scheme entries in source coverage', () => {
    ...
    expect(report.sourceCoverage).toEqual({ verifiedLocalCount: 1, liveOfficialCount: 1, totalSchemesConsidered: 2 })
