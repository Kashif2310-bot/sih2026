        ...
        <ul className="mt-3 space-y-2">
          {byMinistry.map(([id, count]) => (
            <li key={id} className="flex items-center justify-between text-sm">
              <span className="text-ink/80">{kn ? MINISTRIES[id as keyof typeof MINISTRIES].nameKn : MINISTRIES[id as keyof typeof MINISTRIES].name}</span>
...
function Stat({ label, value }: { label: string; value: number }) {
  return (
    <div className="rounded-2xl border border-forest/10 bg-white p-4">
      ...
      <p className="mt-1 font-display text-3xl font-bold text-forest">{value}</p>
    </div>
  )
}
